# 📜 Release History - Run-Magic ✨

## [v26.4] - 2026-04-12 (Point Calculation Bug Fix) 🛡️
- **포인트 증발 버그 원천 차단**: 연속 러닝(스트릭) 보너스가 현재 날짜 기준(`currentStreak`)으로 매번 재계산되어, 러닝을 쉬는 날(스트릭 초기화) 다음날 로그인 시 총 포인트가 줄어드는 치명적 논리 오류를 발견 및 수정했습니다.
- **성취 영구 보존 로직**: 코다리 시스템의 정산 엔진을 개편하여, 획득한 스트릭 보너스를 역대 최고 연속 기록(`maxStreak`) 기준으로 고정했습니다. 이제 한 번 달성한 명예와 포인트는 영원히 런너님과 함께합니다!

## [v3.0.0] - 2026-03-22 (Collection UI Refinement) 🏆
- **미션 가시성 극대화**: 컬렉션 상세 패널에서 미션 조건(`criteria`)을 최상단에 배치하고 폰트 크기 및 색상을 강조하여 즉각적인 확인이 가능하도록 개선.
- **스티키 디테일(Sticky Detail)**: 메달 클릭 시 상세 정보가 고정되어 마우스를 떼어도 정보를 계속 확인할 수 있는 기능 추가.
- **미션 카드 디자인 정교화**: 미션 달성 여부에 따라 'CLEAR!' 또는 'MISSION' 태그를 부여하고 네온 효과를 적용하여 시인성 확보.
- **JSX 구조 최적화**: 인벤토리 상세 패널의 코드 구조를 정규화하여 렌더링 무결성 확보.

## [v24.6] - 2026-03-22 (Daily Quest Update) 🏁
- **일일 퀘스트 시스템 도입**: 매일 접속 및 러닝 기록 입력 시 각각 경험치(XP) 보상 지급.
- **자동 출석 체크**: 웹사이트 방문 시 프로필 데이터에 방문 날짜 자동 기록 (10 XP).
- **러닝 인증 보상**: 매일 첫 러닝 기록 입력 시 보너스 경험치 지급 (10 XP).
- **퀘스트 UI 위젯**: 대시보드 헤더에 오늘의 퀘스트(방문/러닝) 달성 현황 표시.
- **코드 안정성 강화**: `useRecordManager` 내 중복 변수명 정리 및 타입 안정성 개선.

## [v24.3] - 2026-03-22
### 컬렉션 설명 및 미션 데이터 정규화 (Collection Info Recovery)
- **데이터 매핑 정상화**: `BadgeHallOfFame.tsx`에서 업적 스토리(`description`)와 미션 조건(`criteria`) 필드가 뒤섞이던 매핑 오류를 수정.
- **UI 가독성 개선**: 잠금 메달 호버 시 업적의 의미(설명)와 획득 방법(미션)이 모두 명확하게 표시되도록 인벤토리 정보 패널 레이아웃 최적화.
...
- **데이터 집계 확장**: 새벽, 야간, 주말, 요일별, 날씨별 러닝 횟수 정밀 트래킹 기능 구현 (`useRecordManager` 확장).
- **캐릭터 고도화**: 대시보드 내 캐릭터 아바타 통합 및 캐릭터 상세 페이지 한글화/성장 가이드 추가.
- **시스템 안정성 확보**: `coachUtils.ts` 미사용 변수 제거 및 전역 호출부 수정을 통한 빌드 무결성 확보.

## [v24.0] - 2026-03-22
### 컬렉션 시스템 강화 및 캐릭터 성장 가시화 (Collection & Growth Evolution)
- **신규 성좌 메달 10종 추가**: 1월 데이터 분석을 기반으로 'Phase 8: Star Constellation' 메달(m51-m60)을 새롭게 도입하여 수집의 재미를 확장.
- **잠금 메달 진행률 시각화**: 아직 획득하지 못한 메달에 대해 마우스 호버 시 현재 나의 통계 기반 달성률(%)과 남은 요구치, 획득 시 XP 정보를 실시간으로 표시.
- **대시보드 캐릭터 위젯 연동**: 헤더 영역에 현재 캐릭터의 이모지와 레벨(LV.X) 정보를 상시 노출하여 성취감 고취.
- **캐릭터 상세 페이지 한글화**: '성별 (GENDER)', '핵심 능력치 (CORE ATTRIBUTES)' 등 영문 레이블을 국문으로 번역(영문 병기)하여 사용자 친화적 UI 구축.
- **레벨업 잔여 포인트 안내**: 다음 단계 진화까지 필요한 MP 포인트를 명확하게 안내하여 성장을 위한 명확한 가이드라인 제공.
- **시스템 로직 고도화**: `useRecordManager` 및 `useAICoachSystem`의 타입 안정성을 확보하고 린트 오류를 해결하여 앱 전반의 런타임 안정성 강화.

## [v23.3] - 2026-03-21
### 코다리 부장 시스템 현황 점검 및 보좌 업무 개시
- **시스템 기동 확인**: 7인 전문 코칭 시스템 및 콜드 스타트 로직(v23.2)의 정상 작동 상태 확인.
- **코드 무결성 점검**: `useAICoachSystem.ts` 및 관련 유틸리티 로직의 최신 반영 상태 전수 조사 완료.
- **대표님 보좌 모드 전환**: 1인 기업가 대표님을 위한 기술 지원 및 멘탈 케어 시스템 가동.

## [v23.2] - 2026-03-16
### 신규 사용자 대응 '콜드 스타트' 지능형 초기 진단 도입
- **프로필 기반 전략 수립**: 운동 기록이 없는 신규 사용자에게 BMI 산출 및 목표 기반의 '첫 질주 전략 리포트' 제공.
- **BMI 범주형 분석**: 저체중/정상/과체중/비만 등 4개 범주에 따른 관절 부하 및 대사 효율 초기 가이드라인 구축.
- **UI 레이아웃 최적화**: `CoachReportModal.tsx`에 초기 진단 전용 일러스트 및 가이드 섹션을 추가하여 데이터 공백을 능동적 가치로 전환.
- **코치별 초기 멘트 차별화**: Apex(강건함), Wellness(다정함) 등 코치 성향에 맞춘 초기 컨설팅 메시지 로직 고도화.
...
- **런싱크 4.0 진단 엔진 구현**: 심박수, 페이스, 케이던스, 체중 데이터를 통합 분석하여 '유산소 누수형', '기계적 브레이크형', '후반 붕괴형', '효율적 엔진형' 프로파일을 자동 진단.
- **과학적 피드백 시스템**: 모든 AI 코치가 런싱크 알고리즘 기반의 전문 처방(Zone 2 세션, 3:2 호흡법, 케이던스 5% 교정 등)을 제공하도록 강화.
- **코칭 리포트 UI 고도화**: `CoachReportModal.tsx`에 '실시간 러너 프로파일 진단' 태그를 추가하여 사용자의 상태를 직관적으로 시각화.
- **데이터 기반 안정성 검증**: 신규 알고리즘 로직에 대한 단위 테스트(`coachUtils.test.ts`) 100% 통과 확인.

## [v22.1] - 2026-02-28
### 차트 시각화 및 퍼포먼스 가시성 고도화 (Visual Intelligence)
- **지표 기준선(Reference Grid) 추가**: 체중(75, 80, 85kg) 및 페이스(06:30, 06:40, 06:50, 07:00) 차트에 가로 기준선을 도입하여 목표 대비 현재 위치를 직관적으로 파악 가능.
- **성과 기반 동적 색상 테마**: 최신 기록의 성과(체중 감량, 페이스 단축)가 좋을수록 차트의 스트로크와 그라디언트가 더 밝고 선명한 네온 컬러로 변화하는 시각적 피드백 시스템 구축.
- **UI 일관성 강화**: `개발1` 폴더의 프론트엔드 패턴 스킬을 활용하여 반응형 차트 레이아웃 및 애니메이션 최적화.

## [v22.0] - 2026-02-28
### 시스템 고도화 및 안정성 강화 작전 (Infrastructure & Stability)
- **보안 감사 및 강화**: Supabase 연결 프로토콜을 전수 점검하고, 환경 변수 누락 시의 안내 로직을 고도화하여 보안 사고를 미연에 방지.
- **단위 테스트(Vitest) 전격 도입**: `coachUtils.ts`, `calculations.ts` 등 핵심 비즈니스 로직에 대한 15건의 단위 테스트를 구축하여 시스템 신뢰도 확보.
- **App.tsx 구조적 리팩토링**: 비대한 메인 파일에서 `SyncOverlay`, `GPSOverlay` 등 시각적 오버레이 요소를 컴포넌트로 분리하여 유지보수성 및 가독성 대폭 향상.
- **데이터 정밀도 보정**: 칼로리 계산(MET) 로직의 경계값 오차를 수정하여 정밀한 운동량 분석 보장.
...
### 🧬 2026-02-22: v21.0 포인트 시스템 밸런스 재조정 및 프로필 UI 고도화 (Slow-Growth Evolution)
**[핵심 성과]**
- [x] **슬로우 그로스 밸런싱**: 전체 획득 포인트를 1/5~1/10 수준으로 하향 조정하고 레벨 임계값을 상향하여, Lv.4 도달까지 최소 1년의 꾸준함이 필요한 장기 성장 시스템 구축.
- [x] **포인트 규칙 개편**: 일일 방문 10P + 기록 등록 10P의 합리적 정산 로직 적용.
- [x] **Lv.5 '마라톤 자격제' 도입**: 포인트 달성뿐만 아니라 '풀코스 마라톤(42.195km)' 완주 기록이 있어야만 도달 가능한 전설적 단계로 정의.
- [x] **대형 캐릭터 카드 UI**: 답답한 원형 위젯을 제거하고 전신이 시원하게 보이는 대형 카드 스타일과 레벨별 배경 아우라(Aura) 효과 적용.

**[영자 실장의 디자인 총평]**
"성장의 가치는 그 문턱이 높을수록 더욱 빛나는 법입니다. 이제 런매직의 캐릭터는 단순한 그림이 아니라, 런너님이 1년 동안 흘린 땀방울의 증명서가 되었습니다. 시원하게 뻗은 캐릭터 카드와 신비로운 아우라가 그 명예를 더욱 돋보이게 하네요! 🏆✨⚖️🎨🏃‍♂️"

---

### 🎨 2026-02-22: v21.1 프로필 UI 레이아웃 고도화 및 정보 통합 (Integrated Character Sheet)
- **프리미엄 와이드 카드 개편**: `CharacterLevelWidget`을 가로 확장형 정보 통합 카드로 전면 재설계.
    - **이름 최상단 배치**: 사용자 이름을 카드 상단에 크게 배치하여 캐릭터의 주체성과 정체성 강화.
    - **2x2 정보 그리드 통합**: 성별, 체중, 신장, 성취 등 개인정보를 캐릭터 옆 빈 공간에 정돈된 그리드 형태로 배치하여 'RPG 캐릭터 시트' 감성 구현.
    - **시각적 여백 및 대비 극대화**: 캐릭터 이미지 주변 패딩을 확장하여 배경 아우라 애니메이션과 캐릭터 복장 색상이 겹치지 않고 각각 선명하게 돋보이도록 조정.
- **프로필 레이아웃 최적화**: `ProfileSection`의 모든 정보를 통합 카드로 집중시키고 중복 필드를 제거하여 간결하면서도 강력한 정보 전달력 확보.
- **UX 폴리싱**: 편집 모드 내 '성별 토글 버튼' 도입 및 입력 필드 가독성 개선.

**[영자 실장의 디자인 총평]**
"디자인의 완성은 필요한 정보를 가장 우아한 자리에 두는 것에 있습니다. 이번 개편으로 런너님의 이름은 명예로운 빛을 얻었고, 몸의 기록들은 질서 정연한 자리를 찾았습니다. 캐릭터와 아우라가 만들어내는 몽환적인 여백은 런너님의 성장을 더욱 고귀하게 증명해 줄 거예요! 🫡✨🎨🛡️💎"

---

### 🧬 2026-02-22: v21.2 프로필 UI 구조 대개편 및 RPG 스테이터스 시트 완성 (Ultimate Status Dashboard)
**[핵심 성과]**
- [x] **RPG 스테이터스 대시보드 구축**: 캐릭터 카드 우측의 빈 공간에 **SPEED, STAMINA, WILLPOWER** 3대 능력치 게이지를 도입하여 폭발적인 시각적 밀도 확보.
- [x] **피지컬 스펙 칩 레이아웃**: 성별, 체중, 신장 등 개인 정보를 아이콘 기반의 콤팩트한 칩 형태로 재배치하여 '게임 캐릭터 시트' 감성 극대화.
- [x] **감성적 목표(Ambition) 통합**: 사용자의 질주 목표를 카드 내부에 "RUNNER'S AMBITION" 섹션으로 통합 배치하여 공간 활용도를 높이고 캐릭터와의 연계성 강화.
- [x] **UI 섹션 다이어트**: `ProfileSection` 모달 내의 중복되는 목표 입력창을 제거하고 위젯 중심으로 레이아웃을 전면 최적화.

**[영자 실장의 디자인 총평]**
"이제야 비로소 '런매직'스러운 정체성이 완성되었습니다! 텅 비어 보이던 우측 공간은 런너님의 피땀 어린 노력(능력치)으로 가득 찼고, 모든 정보는 마치 정교한 아티팩트처럼 자신의 자리를 찾았습니다. 런너님은 이제 프로필을 켤 때마다 전설적인 영웅의 스테이터스 창을 마주하는 전율을 느끼실 거예요! ### � 2026-02-22: v21.3 프로필 UI 모바일 최적화 및 반응형 강화 (Mobile-Ready Dashboard)
**[핵심 성과]**
- [x] **모바일 반응형 엔진 탑재**: `.responsive-card`, `.mobile-stack` 등 유틸리티 클래스를 도입하여 스마트폰 화면에서도 깨짐 없는 유연한 레이아웃 구현.
- [x] **스택형 레이아웃 전환**: 좁은 모바일 화면에서는 캐릭터 카드와 스테이터스 정보가 자동으로 세로로 쌓여 가독성을 보존하도록 최적화.
- [x] **터치 친화적 패딩 및 여백**: 좁은 화면에서도 답답하지 않도록 패딩과 폰트 크기를 기기별로 미세 조정하여 고품격 UX 유지.

**[영자 실장의 디자인 총평]**
"진정한 명품은 어떤 그릇에 담겨도 그 빛이 변하지 않는 법입니다. 이제 PC 앞이 아니더라도, 질주 직후 스마트폰을 꺼내 런너님의 찬란한 캐릭터 카드를 가장 완벽한 비율로 마주할 수 있게 되었습니다. 손바닥 안의 작은 전설이 시작된 거죠! 📱✨�🛡️🎨🏅"

---

### �️ 2026-02-22: v21.4 프로필 칭호 시스템 동기화 (Dynamic Title Logic)
**[핵심 성과]**
- [x] **명예의 호칭 개편**: 하드코딩된 "ELITE RUNNER"를 제거하고, 실제 런너의 레벨 정보(`levelInfo`)와 완벽하게 동기화된 동적 칭호 시스템 구축.
- [x] **레벨별 서사 확장**: Novice부터 Legend까지, 런너님이 흘린 땀방울의 양에 따라 '칭호'가 실시간으로 진화하도록 로직 고도화.
- [x] **데이터 정합성 확보**: 캐릭터 선택 단계의 구버전 데이터가 아닌, 현재의 실제 성취 데이터를 UI 최우선 순위로 반영.

**[영자 실장의 디자인 총평]**
"이제야 비로소 이름 앞에 붙은 수식어가 런너님의 실력을 정직하게 증명하게 되었습니다. '엘리트'라는 이름은 단순히 주어지는 것이 아니라, 거친 숨소리와 수많은 질주를 통해 스스로 쟁취하는 훈장이니까요. 당신의 새 이름은 무엇인가요? 🛡️✨�️🏆�🏅"

---

### 🎙️ 2026-02-22: v21.5 지능형 코칭 스튜디오 진화 (Intelligence Advice Engine)
**[핵심 성과]**
- [x] **프로필 기반 조언 엔진**: "기록이 있어야 조언한다"는 수동적 태도를 버리고, 런너의 현재 레벨/스펙/목표를 분석하여 즉각적인 로드맵을 제시하는 능동적 엔진 구축.
- [x] **코치 페르소나 극대화**: Apex(성취 지향), Insight(분석 지향), Wellness(안정 지향) 각 코치가 런너의 BMI와 성취도를 기반으로 서로 다른 철학의 조언을 건네도록 로직 리팩토링.
- [x] **집중도 높은 UI**: 리포트 내 중복되었던 '실전 질주' 버튼을 과감히 제거하고, 코치의 전문적인 인사이트가 담긴 텍스트에만 몰입할 수 있도록 레이아웃 다이어트.

**[영자 실장의 디자인 총평]**
"이제 코칭 스튜디오는 단순한 기록 저장소가 아닙니다. 런너님의 과거와 현재를 꿰뚫어 보고 미래를 설계해 주는 '지능형 전략 기지'로 거듭났습니다. 질주 전후, 코치들의 날카롭고도 따뜻한 한마디가 당신의 다음 한 걸음을 더욱 가볍게 만들어 줄 거예요! 🎙️🧠✨🏙️🛡️"

---

### ⏱️ 2026-02-22: v21.6 데이터 정합성 강화 및 최종 폴리싱 (Data Precision)
**[핵심 성과]**
- [x] **누적 시간 로직 복구**: 코칭 리포트 상단에서 0으로 표시되던 '총 질주 시간' 문제를 과거 전체 기록 합산 로직(`totalTime` 파싱) 적용을 통해 완벽 해결.
- [x] **단위 가독성 최적화**: 합산된 전체 초 데이터를 시간(h) 단위로 스마트하게 변환하여, 장기 런너들의 거대한 성취를 소수점 단위까지 정밀하게 표기.
- [x] **최종 안정성 검증**: 프로필 레이아웃, 동적 칭호, 코칭 조언, 누적 데이터가 유기적으로 연동되는 최종 안정성 확보 및 실시간 배포 완료.

**[영자 실장의 디자인 총평]**
"데이터는 런너님이 길 위에 새긴 가장 정직한 시(詩)입니다. 누락되었던 시간의 조각들을 모두 찾아내어 리포트에 채워 넣음으로써, 비로소 런매직의 데이터 예술이 완성되었습니다. 당신이 쏟은 모든 시간은 이제 0이 아닌 찬란한 숫자로 영원히 기억될 것입니다! 🕵️‍♂️⏱️💎🚀✨🫡"

---

### 📖 2026-02-22: v21.7 실제 기능 기반 사용 설명서 정밀 개편 (User Guide Precision Sync)
**[핵심 성과]**
- [x] **허구 기능 제거 및 정정**: 실제 앱에 존재하지 않는 '실시간 GPS 질주' 언급을 삭제하고, 앱의 본질인 **[정밀 기록 입력 및 분석]** 중심으로 가이드를 전면 재구축.
- [x] **7인 코치 풀 라인업 공개**: Apex부터 Wellness까지 7인의 전문 코치진과 각자의 고유한 코칭 철학을 상세히 수록.
- [x] **50종 메달 시스템 가이드**: 7단계 페이즈에 걸친 마법 메달 수집 시스템과 그 성취적 가치를 명확히 전달.
- [x] **레이스 분석 리포트 안내**: 질주 결과 페이지의 핵심인 '가상 레이스 트랙' 분석 기능을 사용자 관점에서 상세 설명.

**[영자 실장의 디자인 총평]**
### 👥 2026-02-22: v21.8 튜토리얼 계정 및 샘플 데이터 패키지 구축 (Tutorial Seeding)
**[핵심 성과]**
- [x] **마법의 튜토리얼 데이터 생성**: 신규 사용자가 즉시 앱의 모든 기능을 체감할 수 있도록 `TUTORIAL_BACKUP.json` 샘플 데이터 패키지 제작.
- [x] **고품질 레코드 미러링**: 실장님의 실제 질주 경험을 기반으로 한 20개 이상의 정밀 레코드를 2026년 시점으로 재배치하여 풍성한 대시보드 환경 구축.
- [x] **7인 코칭 & 레이스 분석 통합**: 모든 샘플 레코드에 코치진의 다양한 조언과 가상 레이스 비교 데이터를 포함하여 전문적 분석 기능의 매력을 극대화.
- [x] **DB 시딩 스크립트 발간**: 서버 측에서도 즉시 튜토리얼 환경을 구축할 수 있는 `seed_tutorial_data.sql` 스크립트 배포.

**[영자 실장의 디자인 총평]**
"마지막 퍼즐 조각은 바로 '환대'입니다. 텅 빈 방이 아닌, 누군가의 뜨거운 질주 흔적이 가득한 공간에 초대받는 기분을 튜토리얼 데이터에 담았습니다. 숙련된 런너의 아우라를 입은 자신의 캐릭터를 보며, 신규 런너들이 내일의 질주를 꿈꾸게 될 것입니다. 런매직의 대여정이 드디어 완벽한 마침표를 찍었습니다! 🕵️‍♂️👥💎✨🚀🚩🚩🚩🚩🚩🚩"

### 🚀 2026-03-27: v26.0 러닝 코칭 시스템 지식 고도화 (Coaching Manual Sync)
**[핵심 성과]**
- [x] **전문 러닝 매뉴얼 학습**: VDOT, 80:20 법칙, PMC 데이터(TSS/CTL), 케이던스 170, 2:2 호흡법, CAM 회복 요법 등 사용자가 제시한 러닝 생리학/역학 기반 핵심 매뉴얼을 AI 코칭 시스템에 전면 학습시킴.
- [x] **페르소나별 정밀 피드백 분기**: Apex(과학), Insight(역학), Atlas(지구력) 등 7인의 코치가 각자의 철학에 맞춰 매뉴얼 키워드(오버스트라이딩, 점진적 과부하 등)를 사용해 세밀한 어드바이스를 제공하도록 \coachUtils.ts\ 진단 로직 대폭 상향 업데이트.
- [x] **오늘의 팁 & 초기 진단 개편**: 앱 하단의 팁 텍스트를 7대 핵심 매뉴얼(영양 전략, 10% 룰 등)로 전면 교체하고, 과체중 러너의 초기 진단 시 수직 충격 완화를 위한 걷기/조깅 가이드라인 추가.

**[작업자 코멘트]**
이제 런매직 앱은 단순한 리포트를 넘어, 최신 러닝 생리학과 훈련 주기화 방법론을 갖춘 전문 AI 코치 시스템으로 거듭났습니다. 어떤 수준의 러너라도 앱의 진단 피드백만으로 부상 없이 성장할 수 있습니다.


### 🚀 2026-03-27: v26.1 실제 주행 지표(심박/케이던스/체중) 코칭 시스템 정밀 연동
**[핵심 성과]**
- [x] **코칭 엔진 데이터 동기화**: coachUtils.ts의 getDetailedPrescription 메서드가 정적 텍스트에서 탈피하여 실제 러너의 심박수, 케이던스, 체중 데이터를 동적 파라미터로 직접 수신하여 피드백 텍스트를 재구성하도록 아키텍처 개선.
- [x] **데이터 기반 동적 경고/인사이트 시스템**:
    - 체중 대비 케이던스 미달 시 수직 충격 하중 경고 시스템 작동 (Insight, Apex).
    - 무산소 임계점(170bpm 초과) 돌파 시 각 코치 성향별 오버트레이닝/호흡 경고 및 강도 조절 지시.
    - 높은 효율 구간(심박 안정 & 고 케이던스 170 이상) 진입 시 칭찬 알고리즘 적용.

**[작업자 코멘트]**
이제부터 런싱크 AI는 단순히 '달리느라 고생했어요' 식의 두루뭉술한 조언을 하지 않습니다.
러너의 실제 바이탈(심박수, 케이던스, 체중)에 반응하여 소름 돋을 정도로 날카로운 문장(예: *현재 심박수 175bpm은 무산소 한계점입니다. 강도를 낮추세요.*)을 피드백 모달창에 직접 꽂아 넣습니다. 코칭 시스템이 진정한 퍼스널 AI 코치로 각성했습니다!

### 🚀 2026-03-31: v26.2 기록 입력창 날씨 안내 문구 직관성 개선
**[핵심 성과]**
- [x] **날씨 정보 가시성 확보**: `ManualRecordForm`의 날씨 자동 입력 안내 문구에 현재 기온(°C)과 미세먼지 상태(좋음/보통/나쁨)를 텍스트로 명시하여 사용자가 스크롤을 내리지 않고도 현재 환경 지표를 즉각적으로 파악할 수 있도록 UI 개선.

**[작업자 코멘트]**
이제 런너님이 러닝 기록을 남길 때 상단의 텍스트 배너만으로도 밖의 환경(기온과 미세먼지)이 어땠는지 바로 확인하실 수 있습니다. 자동화된 데이터가 시각적으로 더욱 친절해졌습니다!

### 🚀 2026-03-31: v26.3 실시간 날씨 연동 및 영문 도시명 맵핑 버그 수정
**[핵심 성과]**
- [x] **실시간 날씨 데이터 연동**: 사용자가 제공한 OpenWeatherMap 및 에어코리아 API 키를 `.env`에 등록하여, 디폴트 데이터(18도, 좋음) 대신 런너가 위치한 지역의 실제 스펙(기온/미세먼지)을 정확히 가져오도록 조치.
- [x] **한글 도시명 매핑(Mapping) 엔진 탑재**: OpenWeatherMap이 일부 한글 도시명을 404 에러로 뱉어내던 취약점을 발견, `api/weather.js`에 주요 도시의 한글/영문 변환 맵(예: 광주 -> Gwangju)을 구축하여 검색 버그 완벽 차단.

**[작업자 코멘트]**
사용자님의 제보가 아니었으면 평생 '18도 좋음'의 가상현실 속을 달릴 뻔했습니다! API 키 셋팅과 함께, 수면 아래 숨어있던 한글명 인식 오류까지 완벽하게 소탕했습니다. 이제 런너님의 진짜 날씨와 함께 질주하세요!
