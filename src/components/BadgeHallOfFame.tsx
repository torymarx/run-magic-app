...
const BadgeHallOfFame: React.FC<BadgeHallOfFameProps> = ({ unlockedBadges, unlockedMedals, medalAchievements, totalStats }) => {
    ...
    const getRemainingGuide = (item: any, stats: any) => {
        if (!stats || !item.targetValue) return null;
        const current = stats[item.category] || 0;
        const target = item.targetValue;
        const diff = target - current;

        if (diff <= 0 && item.category !== 'pace' && item.category !== 'bestPace') return null;

        if (item.category === 'distance') {
            const remaining = Math.max(0, diff).toFixed(1);
            return `달성까지 ${remaining}km 남음`;
        }
        if (item.category === 'time') {
            const remaining = Math.max(0, diff);
            return `달성까지 ${remaining}분 남음`;
        }
        if ([
            'sessions', 'dawnCount', 'nightCount', 'weekendCount', 'mondayCount', 
            'shortRunCount', 'earlyCount', 'lateCount', 'stormCount'
        ].includes(item.category)) {
            const remaining = Math.max(0, diff);
            return `달성까지 ${remaining}회 남음`;
        }
        if (item.category === 'streak') {
            const remaining = Math.max(0, diff);
            return `달성까지 ${remaining}일 남음`;
        }
        if (item.category === 'pace') {
            if (current === 9999 || current <= 0) {
                const targetMin = Math.floor(target / 60);
                const targetSec = target % 60;
                return `목표 페이스: ${targetMin}'${targetSec.toString().padStart(2, '0')}" (첫 기록 등록 필요)`;
            }
            const paceDiff = current - target;
            if (paceDiff > 0) {
                return `페이스 ${paceDiff}초 단축 필요`;
            }
            return null;
        }
        if (item.category === 'special') {
            switch (item.id) {
                case 'm1': return '프로필 설정을 완료해 주세요';
                case 'm3': return '단일 세션 10분 이상 지속 필요';
                case 'm10': return '단일 세션 3km 이상 완주 필요';
                case 'm14': return '단일 세션 7km 이상 완주 필요';
                case 'm15': return '단일 세션 30분 이상 지속 필요';
                case 'm17':
                case 'm62': return '단일 세션 5km 이상 완주 필요';
                case 'm20': return '단일 세션 10km 이상 완주 필요';
                case 'm37': return '매월 5회 이상, 6개월 유지 필요';
                case 'm44': return '봄/여름/가을/겨울 각 계절 10회 달성 필요';
                case 'm48': return '가입 1주년 및 누적 100회 달성 필요';
                case 'm50': return '1,000km & 10,000분 & 365회 달성 필요';
                case 'm61': return '단일 세션 4km 이상 완주 필요';
                case 'm63': return '단일 세션 6km 이상 완주 필요';
                case 'm67': return '10km 1시간 이내 완주 필요';
                default: return `달성 조건 충족 필요`;
            }
        }
        return null;
    };
                                    ...
                                    <>
                                        {totalStats && activeItem.targetValue && (
                                            <div style={{ marginTop: '4px' }}>
                                                ...
                                                {/* v24.7: 남은 수치 직관화 가이드 추가 */}
                                                {(() => {
                                                    const guideText = getRemainingGuide(activeItem, totalStats);
