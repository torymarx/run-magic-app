...
const ManualRecordForm: React.FC<ManualRecordFormProps> = ({
    ...
    const [frequentTimes, setFrequentTimes] = useState<string[]>(() => {
        const saved = localStorage.getItem('frequent_times');
        return saved ? JSON.parse(saved) : ['06:30', '19:30'];
    });

    const addFrequentTime = (e: React.MouseEvent) => {
        e.preventDefault();
        if (!frequentTimes.includes(time)) {
            const newTimes = [...frequentTimes, time].sort();
            setFrequentTimes(newTimes);
            localStorage.setItem('frequent_times', JSON.stringify(newTimes));
        }
    };

    const removeFrequentTime = (t: string, e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        const newTimes = frequentTimes.filter(item => item !== t);
        setFrequentTimes(newTimes);
        localStorage.setItem('frequent_times', JSON.stringify(newTimes));
    };
    // v25.0: 날씨 자동 입력 시스템
    ...
    const handleAddSplit = () => {
        // v32.0: 내부적으로는 모두 {거리}km@{시간} 형식으로 관리하여 정밀도 향상
        setSplits([...splits, '1.0km@06:00']);
        setDistance(Number((distance + 1.0).toFixed(2)));
    };

    // v32.0: 짜투리 구간(1km 미만) 추가 기능
    const handleAddFractionalSplit = () => {
        setSplits([...splits, '0.5km@03:00']);
        setDistance(Number((distance + 0.5).toFixed(2)));
    };

    const handleRemoveSplit = (index: number) => {
        const splitToRemove = splits[index];
        let distToRemove = 1.0;
        if (splitToRemove.includes('km@')) {
            distToRemove = parseFloat(splitToRemove.split('km@')[0]);
        }
        
        const newSplits = splits.filter((_, i) => i !== index);
        setSplits(newSplits);
        setDistance(Number(Math.max(0, distance - distToRemove).toFixed(2)));
    };

    const handleSplitChange = (index: number, value: string, distOverride?: number) => {
        const newSplits = [...splits];
        const currentSplit = splits[index];
        
        let currentDist = 1.0;
        let currentTime = currentSplit;

        if (currentSplit.includes('km@')) {
            const parts = currentSplit.split('km@');
            currentDist = parseFloat(parts[0]);
            currentTime = parts[1];
        }

        if (distOverride !== undefined) {
            // 거리 수정 시
            const oldDist = currentDist;
            currentDist = distOverride;
            newSplits[index] = `${currentDist}km@${currentTime}`;
            setDistance(Number((distance - oldDist + currentDist).toFixed(2)));
        } else {
            // 시간 수정 시
            ...
            newSplits[index] = `${currentDist}km@${formattedValue}`;
        }
    ...
    const handleDateChange = async (newDate: Date) => {
        ...
        // v34.0: 달력 날짜 클릭 시 기존 기록 유무와 상관없이 항상 '신규 입력' 모드로 시작
        // 기존 기록 수정은 달력 아래의 리스트 항목을 클릭했을 때만 활성화됨
        handleResetToNew(newDate);
    };
                    ...
                    <form onSubmit={handleSubmit}>
                        <div style={{ marginBottom: '1.5rem' }}>
                            ...
                            <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                                ...
                                />
                                {!frequentTimes.includes(time) && (
                                    <button
                                        onClick={addFrequentTime}
                                        style={{
                                            padding: '0.8rem',
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginTop: '0.5rem' }}>
                                {frequentTimes.map((t) => (
                                    <div
                                        ...
                                        style={{
                                            padding: '4px 10px',
                                            background: time === t ? 'var(--electric-blue)' : 'rgba(255,255,255,0.05)',
                                            border: '1px solid',
                                            borderColor: time === t ? 'var(--electric-blue)' : 'rgba(255,255,255,0.1)',
                                            borderRadius: '12px',
                                            fontSize: '0.75rem',
                                            color: time === t ? 'black' : 'white',
                                            cursor: 'pointer',
                                            display: 'flex',
                                            alignItems: 'center',
                                            gap: '4px',
                                    ...
                                    >
                                        {parseInt(t.split(':')[0]) < 12 ? '🌅' : '🌇'} {t}
                                        <X
                                            size={10}
                                            onClick={(e) => removeFrequentTime(t, e)}
                            ...
                            <div style={{ padding: '1rem', background: 'rgba(0, 209, 255, 0.02)', borderRadius: '12px', border: '1px solid rgba(0, 209, 255, 0.1)' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                                    {splits.map((split, index) => {
                                        const isFractional = split.includes('km@');
                                        const displayTime = isFractional ? split.split('km@')[1] : split;
                                        const distValue = isFractional ? parseFloat(split.split('km@')[0]) : 1.0;

                                        return (
                                            <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                                {isFractional ? (
                                                    <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(255,215,0,0.05)', borderRadius: '8px', padding: '0 0.4rem', border: '1px solid rgba(255,215,0,0.1)' }}>
                                                        <input
                                                            type="number"
                                                            step="0.01"
                                                            value={distValue}
                                                            onChange={(e) => handleSplitChange(index, displayTime, parseFloat(e.target.value))}
                                                            style={{ width: '45px', background: 'transparent', border: 'none', color: '#FFD700', fontSize: '0.8rem', textAlign: 'center', outline: 'none', padding: '0.4rem 0' }}
                                                        />
                                                        <span style={{ fontSize: '0.7rem', color: '#FFD700', opacity: 0.7 }}>km</span>
                                                    </div>
                                                ) : (
                                                    <span style={{ fontSize: '0.8rem', width: '50px', opacity: 0.7, textAlign: 'center' }}>1.0km</span>
                                                )}
                                                ...
                                                <input
                                                    ...
                                                    onFocus={(e) => {
                                                        if (displayTime === '06:00' || displayTime === '03:00') {
                                                    ...
                                                    onKeyDown={(e) => {
                                                        if (e.key === 'Enter') {
                                                            e.preventDefault();
                                                            const nextInput = splitRefs.current[index + 1];
                                                            if (nextInput) {
                                                                nextInput.focus();
                                                            }
                                                        }
                                ...
                                <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
                                    ...
                                    >
                                        <Plus size={14} /> 구간 추가 (1km)
                                    </button>
                                    <button
                                        type="button"
                                        onClick={handleAddFractionalSplit}
                                        style={{
                                            flex: 1,
