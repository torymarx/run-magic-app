...
const BioPerformanceChart: React.FC<BioPerformanceChartProps> = ({ records, viewingDate, onDateChange }) => {
    ...
    const performanceAnalysis = useMemo(() => {
        ...
        // v36.0: 당일 기록이 여러 개일 경우 합산 및 최적 데이터 추출
        const latestDate = chartData[chartData.length - 1].fullDate;
        const todayRecords = chartData.filter(d => d.fullDate === latestDate);
        
        // 거리: 합산
        const todayTotalDist = todayRecords.reduce((acc, curr) => acc + curr.distance, 0);
        
        // 페이스: 최우수 (최저 초/km)
        const todayBestPaceSec = Math.min(...todayRecords.map(r => r.paceSeconds));
        
        // 심박수: 최우수 (최저 심박수 - 심폐 효율 기준)
        const hrValues = todayRecords.filter(r => r.hr && r.hr > 0).map(r => (r.hr as number));
        const todayBestHR = hrValues.length > 0 ? Math.min(...hrValues) : null;
        
        // 케이던스: 최우수 (최고 케이던스 - 주행 효율 기준)
        const cadValues = todayRecords.filter(r => r.cad && r.cad > 0).map(r => (r.cad as number));
        const todayBestCadence = cadValues.length > 0 ? Math.max(...cadValues) : null;

        const todayAvgWeight = todayRecords.reduce((acc, curr) => acc + curr.weight, 0) / todayRecords.length;

        // 월간 평균 데이터 계산 (일평균 기준)
        const uniqueDates = Array.from(new Set(chartData.map(d => d.fullDate)));
        const totalMonthlyDist = chartData.reduce((acc, curr) => acc + curr.distance, 0);
        const avgMonthlyDistPerDay = totalMonthlyDist / uniqueDates.length;

        const totalMonthlyTime = chartData.reduce((acc, curr) => acc + curr.totalTimeSeconds, 0);
        const avgMonthlyPaceSec = totalMonthlyDist > 0 ? totalMonthlyTime / totalMonthlyDist : 0;
        const avgMonthlyWeight = chartData.reduce((acc, curr) => acc + curr.weight, 0) / chartData.length;

        const paceDiffPercent = avgMonthlyPaceSec > 0 ? ((todayBestPaceSec - avgMonthlyPaceSec) / avgMonthlyPaceSec) * 100 : 0;
        const distDiffPercent = avgMonthlyDistPerDay > 0 ? ((todayTotalDist - avgMonthlyDistPerDay) / avgMonthlyDistPerDay) * 100 : 0;
        const weightDiffPercent = avgMonthlyWeight > 0 ? ((todayAvgWeight - avgMonthlyWeight) / avgMonthlyWeight) * 100 : 0;

        return {
            today: todayRecords[todayRecords.length - 1], 
            todayPaceStr: formatPace(todayBestPaceSec),
            todayDistStr: todayTotalDist.toFixed(1),
            todayWeightStr: todayAvgWeight.toFixed(1),
            todayBestHR,
            todayBestCadence,
            avgDist: avgMonthlyDistPerDay,
            avgPaceSec: avgMonthlyPaceSec,
            avgWeight: avgMonthlyWeight,
            paceDiffPercent,
            distDiffPercent,
            weightDiffPercent
        };
                ...
                <div style={{
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem' }}>
                        ...
                        <div style={{ background: 'rgba(0,0,0,0.2)', padding: '16px', borderRadius: '20px', display: 'flex', flexDirection: 'column', gap: '8px', border: '1px solid rgba(57, 255, 20, 0.1)' }}>
                            <span style={{ fontSize: '0.75rem', opacity: 0.5, fontWeight: 'bold' }}>오늘 러닝 거리</span>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
